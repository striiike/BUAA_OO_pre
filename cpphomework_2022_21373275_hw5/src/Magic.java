...
public class Magic {
    ...
    public static void main(String[] args) {
        ...
        while (scanner.hasNext()) {
            ...
            if (Objects.equals(quireOp, "qdate")) {
                inquireDate(scanner, messages);
            }
            // inquire sender
            if (Objects.equals(quireOp, "qsend")) {
                inquireSend(scanner, messages);

            }
            // inquire receiver
            if (Objects.equals(quireOp, "qrecv")) {
                inquireRecv(scanner, messages);
            }
        }
    }

    public static void inquireDate(Scanner scanner, ArrayList<Message> messages) {
        String temp = scanner.next();
        StringBuilder str = new StringBuilder("0000" + temp);
        ...
        for (Message item : messages) {
            if (year == -1 || year == item.getYear()) {
                if (month == 0 || month == item.getMonth()) {
                    if (day == 0 || day == item.getDay()) {
    ...
    public static void inquireSend(Scanner scanner, ArrayList<Message> messages) {
        String str = scanner.next();
        // judge if vague
        if (Objects.equals(str, "-v")) {
            str = scanner.next();
            for (Message item : messages) {
                if (item.getSender().contains(str.substring(1, str.length() - 1))) {
                    System.out.println(item.getMessage() + ';');
                }
            }
            return;
    ...
    public static void inquireRecv(Scanner scanner, ArrayList<Message> messages) {
        String str = scanner.next();
        // judge if vague
        if (Objects.equals(str, "-v")) {
            str = scanner.next();
            for (Message item : messages) {
                if (item.getReceiver() != null) {
                    if (item.getReceiver().contains(str.substring(1, str.length() - 1))) {
                        System.out.println(item.getMessage() + ';');
                    }
                }
            }
